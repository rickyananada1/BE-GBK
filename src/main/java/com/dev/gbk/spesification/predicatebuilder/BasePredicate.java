...
import static com.dev.gbk.utils.SpecificationUtils.getCompoundKeys;
import static com.dev.gbk.utils.SpecificationUtils.isKeyCompound;
...
public abstract class BasePredicate<T> implements Specification<T> {
    ...
    protected Object getCriteriaObjectValue() {
        return searchCriteria.isValueSearchCriteria() ? ((SearchCriteria) searchCriteria.getValue()).getValue()
    ...
    private Comparable<? super Object> parseJavaType(String value, Class<?> clazz) {
        ...
        try {
            parsedValue = (Comparable<? super Object>) objectMapper.readValue(objectMapper.writeValueAsBytes(value),
                    clazz);
        } catch (Exception ex) {
